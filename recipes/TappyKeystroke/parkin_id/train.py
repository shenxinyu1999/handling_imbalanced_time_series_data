...
from tappy_keystroke_prepare import prepare_tappy_keystroke
...
class LID(sb.Brain):
    ...
    def compute_objectives(self, inputs, batch, stage):
        ...
        targets = batch.parkinson_encoded.data
...
def dataio_prep(hparams):
    ...
    parkinson_encoder = sb.dataio.encoder.CategoricalEncoder()

    # Define audio pipeline
    @sb.utils.data_pipeline.takes("txt")
    @sb.utils.data_pipeline.provides("sig")
    def audio_pipeline(txt):
    ...
    @sb.utils.data_pipeline.takes("parkinson")
    @sb.utils.data_pipeline.provides("parkinson", "parkinson_encoded")
    def label_pipeline(parkinson):
        yield parkinson
        parkinson_encoded = parkinson_encoder.encode_label_torch(parkinson)
        yield parkinson_encoded
    ...
    for dataset in ["train", "test"]:
        datasets[dataset] = sb.dataio.dataset.DynamicItemDataset.from_csv(
            ...
            output_keys=["id", "sig", "parkinson_encoded"],
        )

    # Load or compute the label encoder (with multi-GPU DDP support)
    # Please, take a look into the lab_enc_file to see the label to index
    # mappinng.
    parkinson_encoder_file = os.path.join(
        hparams["save_folder"], "parkinson_encoder.txt"
    )
    parkinson_encoder.load_or_create(
        path=parkinson_encoder_file,
        from_didatasets=[datasets["train"]],
        output_key="parkinson",
    )

    return datasets, parkinson_encoder
...
if __name__ == "__main__":
    ...
    datasets, parkinson_encoder = dataio_prep(hparams)
